	...
	void DynamicBoundaries::SetupFromDictionary(OpenSMOKE::OpenSMOKE_Dictionary& dictionary)
	{
		...
		if (dictionary.CheckOption("@Type") == true)
		{
			...
			else
			{
				...
				const std::string message2 = "Available types: TEMPERATURE_VELOCITY_SLOPES | FIXED_STRAINRATE_FUEL_TEMPERATURE_SLOPE | FIXED_STRAINRATE_OX_TEMPERATURE_SLOPE | FIXED_BALANCE_FUEL_VELOCITY_SLOPE | FIXED_BALANCE_OX_VELOCITY_SLOPE";
		...
		// TODO
		/*
		if (dictionary.CheckOption("@SemiAmplitude") == true)
		{
			dictionary.ReadDouble("@SemiAmplitude", semi_amplitude_);
		}

		if (dictionary.CheckOption("@Frequency") == true)
		{
			double value;
			std::string units;
			dictionary.ReadMeasure("@Frequency", value, units);
			if (units == "1/s")			frequency_ = value;
			else if (units == "Hz")		frequency_ = value;
